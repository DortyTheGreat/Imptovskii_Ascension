...
public class AutoTrapPlus extends Module {
    public AutoTrapPlus(){
        super(Addon.ProximaCombat, "auto-trap-plus", "Fixed at 15-08-2023, by Dorty for 1.20.1b IA release. Made from some sources");
    }
                    ...
                    for (Direction direction1 : directions){
                        ...
                        if (EntityUtils.intersectsWithEntity(box1, entity -> entity instanceof PlayerEntity)){
                            for (CardinalDirection direction2 : CardinalDirection.values()) {
                                ...
                                if (!EntityUtils.intersectsWithEntity(box2, entity -> entity instanceof PlayerEntity || entity instanceof EndCrystalEntity)  && BlockUtil.distance(mc.player.getEyePos(), pos2) <= placeRange.get()  && (BlockUtil.isAir(pos2) || BlockUtil.getBlock(pos2) == Blocks.LAVA  || BlockUtil.getBlock(pos2) == Blocks.WATER  || BlockUtil.getBlock(pos2) == Blocks.FIRE) && !array.contains(pos2)) array.add(pos2);
                            }
                        }
                        else {
                            if (BlockUtil.distance(mc.player.getEyePos(), pos1) <= placeRange.get() && (BlockUtil.isAir(pos1) || BlockUtil.getBlock(pos1) == Blocks.LAVA  || BlockUtil.getBlock(pos1) == Blocks.WATER  || BlockUtil.getBlock(pos1) == Blocks.FIRE) && !array.contains(pos1)) array.add(pos1);
                        }
                    }
                }else {
                    if (BlockUtil.distance(mc.player.getEyePos(), pos) <= placeRange.get() && (BlockUtil.isAir(pos) || BlockUtil.getBlock(pos) == Blocks.LAVA  || BlockUtil.getBlock(pos) == Blocks.WATER  || BlockUtil.getBlock(pos) == Blocks.FIRE) && !array.contains(pos))  array.add(pos);
                }
            }
            try {
                if (array.isEmpty()) return;
                for (BlockPos pos : array){
                    if (BlockUtil.distance(mc.player.getEyePos(), pos) > placeRange.get()){
                        array.remove(pos);
                    }
