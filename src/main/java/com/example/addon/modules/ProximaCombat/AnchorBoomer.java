...
import com.example.addon.utils.BlockUtil;
import com.example.addon.utils.EntityUtil;
...
public class AnchorBoomer extends Module {
    ...
    private void onTick(TickEvent.Pre event){
        ...
        switch (targetMode.get()){
            case Nearest -> playerArray.sort(Comparator.comparing(player -> BlockUtil.distance(mc.player.getEyePos(), player.getBlockPos())));
        ...
        if ((breakTop.get().isPressed() || autoMineTop.get()) && !(BlockUtil.getBlock(playerArray.get(0).getBlockPos().up(2)) instanceof RespawnAnchorBlock) && !BlockUtil.isAir(playerArray.get(0).getBlockPos().up(2)) && BlockUtil.getState(playerArray.get(0).getBlockPos().up(2)).getHardness(mc.world, playerArray.get(0).getBlockPos().up(2)) != -1 && BlockUtil.distance(mc.player.getEyePos(),  playerArray.get(0).getBlockPos().up(2)) <= placeRange.get()){
        ...
        if (EntityUtil.isInHole(false, target)){
        ...
        else if (isSafeFromFacePlace(target) && !EntityUtil.isInHole(false, target) && BlockUtil.distance(mc.player.getEyePos(), target.getBlockPos()) <= placeRange.get()){
            ...
            array = new ArrayList<>(){{
                ...
                sur.sort(Comparator.comparing(block -> BlockUtil.distance(mc.player.getEyePos(), block)));
                if (!sur.isEmpty()){
                    add(sur.get(0));
                }
            }};
    ...
    private List<BlockPos> sortBedPos(List<BlockPos> array, PlayerEntity target){
        ...
        switch (sortMode.get()) {
            case CloseToPlayer -> firstArray.sort(Comparator.comparing(bedBlock -> BlockUtil.distance(mc.player.getEyePos(), bedBlock)));
    ...
    private List<BlockPos> getPosAround(){
        return BlockUtil.getSphere(mc.player.getEyePos(), xPlaceRange.get().floatValue(), yPlaceRange.get().intValue(), false, true, 0).stream().filter(this::canAnchorPlace).collect(Collectors.toList());
    }
    ...
    private BlockPos findBreak(PlayerEntity target){
        List<BlockPos> array = BlockUtil.getSphere(mc.player.getEyePos(), xBreakRange.get().floatValue(), yBreakRange.get().intValue(), false, true, 0).stream().filter(blockPos -> BlockUtil.getBlock(blockPos) instanceof RespawnAnchorBlock).collect(Collectors.toList());
        ...
        array = array.stream().filter(blockPos -> {
            ...
            if (lethalDamage.get() && PlayerUtils.getTotalHealth() - DamageUtils.bedDamage(mc.player, anchorVec) < minHealth.get()) return false;
            return true;
    ...
    private boolean isSelfTraped(PlayerEntity player){
        ...
        selftrapArray = selftrapArray.stream().filter(blockPos -> BlockUtil.distance(mc.player.getEyePos(), blockPos) < placeRange.get()).collect(Collectors.toList());
