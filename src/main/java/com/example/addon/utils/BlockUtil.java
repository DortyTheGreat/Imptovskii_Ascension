...
public class BlockUtil {
    ...
    public static List<Direction> getVisibleBlockSides(Vec3d pos, BlockPos blockPos) {
        ...
        sides.removeIf(side -> !mc.world.getBlockState(blockPos.offset(side)).isReplaceable());
    ...
    public static boolean isVecComplete(ArrayList<Vec3d> vlist) {
        ...
        for (Vec3d b : vlist) {
            BlockPos bb = ppos.add( Vec3Util.Vec3dToVec3i (b) );
    ...
    public static List<BlockPos> getSphere(Vec3d centerPos, int radius, int height) {
        ArrayList<BlockPos> blocks = new ArrayList<>();
        for (int i = (int) centerPos.getX() - radius; i < centerPos.getX() + radius; i++) {
            for (int j = (int) centerPos.getY() - height; j < centerPos.getY() + height; j++) {
                for (int k = (int) centerPos.getZ() - radius; k < centerPos.getZ() + radius; k++) {
    ...
    public static List<BlockPos> getSphere(Vec3d loc, float r, int h, boolean hollow, boolean sphere, int plus_y) {
	...
	public static double distance(Vec3d block1, BlockPos block2) {
        return distance(block1, new Vec3d(block2.getX(), block2.getY(), block2.getZ()));
    }
    ...
    public static void rightClickBlock(BlockPos pos, Vec3d vec, Hand hand, Direction direction, boolean packet, boolean swing) {
        if (packet) {
            mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(hand, new BlockHitResult(vec, direction, pos, true), 0 /* я не уверен, что тут должен быть 0, но в метеоре обновили имеено так*/));
        } else {
            mc.interactionManager.interactBlock(mc.player, hand, new BlockHitResult(vec, direction, pos, true));
        }
        if (swing) {
